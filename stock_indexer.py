...
def process_stock_data(file_path):
    ...
    Each ticker gets its own independent index from 1 to N (where N = number of records for that ticker).
    ...
    for ticker in df_indexed['Ticker'].unique():
        ...
        if len(duplicates) > 0:
            ...
            if ticker_max != max_records_per_ticker:
                print(f"ERROR: {ticker} should end at {max_records_per_ticker} but ends at {ticker_max}")
                duplicate_check_passed = False
...
def verify_core_data_integrity(original_df, processed_df):
    ...
    if len(original_df) != len(processed_df):
        print(f"ERROR: Row count changed! Original: {len(original_df)}, Processed: {len(processed_df)}")
        return False
...
def verify_index_integrity(df):
    ...
    for ticker in df['Ticker'].unique():
        ...
        if ticker_max != global_max:
            print(f"ERROR: {ticker} should end at {global_max} but ends at {ticker_max}")
            issues_found = True
            continue
        ...
        if expected_indices != actual_indices:
            ...
            issues_found = True
            continue
        print(f"✓ {ticker}: {n_records} records, indices {expected_start} to {global_max}")

    if not issues_found:
        print("✓ All tickers end at the global maximum with correct sequential indices!")
        return True
    else:
...
def main():
    ...
    try:
        ...
        unnamed_cols = [col for col in original_df_raw.columns if 'Unnamed:' in str(col)]
        if unnamed_cols:
        ...
        for ticker in example_tickers:
            ...
            if len(ticker_data) > 8:
                print("...")
                print(ticker_data[['Ticker', 'Index', 'Report', 'EPS', 'Revenue']].tail(3))
        ...
        print("✓ Most recent record for each ticker has the highest index for that ticker")
