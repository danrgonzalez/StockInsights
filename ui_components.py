...
def display_summary_stats(df, ticker):
    ...
    with col2:
        latest_eps = (
            ...
            if not ticker_data['EPS'].dropna().empty else "N/A"
        )
        latest_eps_ttm = (
            ticker_data['EPS_TTM'].dropna().iloc[-1]
            if (
                'EPS_TTM' in ticker_data.columns and
                not ticker_data['EPS_TTM'].dropna().empty
            )
            else "N/A"
        )
        if latest_eps != "N/A" and latest_eps_ttm != "N/A":
            st.metric(
                "Latest EPS", f"{latest_eps:.2f}",
                help=f"TTM: {latest_eps_ttm:.2f}"
            )
        elif latest_eps != "N/A":
    ...
    with col3:
        latest_revenue = (
            ...
            if not ticker_data['Revenue'].dropna().empty else "N/A"
        )
        latest_revenue_ttm = (
        ...
        if latest_revenue != "N/A" and latest_revenue_ttm != "N/A":
            st.metric(
                "Latest Revenue", f"${latest_revenue:,.0f}M",
    ...
    with col4:
        latest_price = (
            ...
            if not ticker_data['Price'].dropna().empty else "N/A"
        )
        latest_multiple = (
